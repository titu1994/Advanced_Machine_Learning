...
def load_model_params():
    np.random.seed(0)
    weights = np.random.uniform(-0.5, 0.5, size=(26, 128))
    bias = np.zeros((26, 1), dtype='float64')

    W = np.concatenate((weights, bias), axis=-1)
    T = np.random.uniform(-0.5, 0.5, size=(26, 26))

    params = np.concatenate((W.flatten(), T.flatten()))
    return np.array(params)

def load_model_params_zeros():
    params = np.zeros((26 * 129 + 26 * 26,))
    return params
