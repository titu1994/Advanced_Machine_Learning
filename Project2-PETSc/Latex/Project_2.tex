...
The definition of "number of effective passes" would change due to the introduction of sampling. In the previous case where exact inference in a linear-chain CRF was possible, the "number of effective passes" would accurately tell you the number of iterations the optimizer would converge on a function value. In the case of sampling, this definition would need to be changed to accommodate for the extra operation of sampling. Convergence would be slower (still converging around the actual minimum function value) and could be slightly erratic due to the nature of sampling. Sampling also includes the goal of estimating the posterior distribution which would take longer than exact inference.
