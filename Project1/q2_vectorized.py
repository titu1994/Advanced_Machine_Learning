...
def forward_backward_pass(X_train, Wj, Tij):
    start = timeit.default_timer()

    N = X_train.shape[0]

    alpha = np.zeros([N, Wj.shape[0]])  # init alpha values to zeros
    alpha[0,] = 1  # init first alpha[Xj,]  to 1

    beta = np.zeros([N, Wj.shape[0]])
    beta[-1,] = 1  # init last beta[Xj,] to 1
    ...
    for j_ in range(1, N):
        ...
        if X_train[j_forward][0] != X_train[j_forward - 1][0] or X_train[j_backward][0] != X_train[j_backward + 1][0]:
            if X_train[j_forward][0] != X_train[j_forward - 1][0]:
                alpha[j_forward,] = 1  # init alpha[Xj,] to 1 if new word
            if X_train[j_backward][0] != X_train[j_backward + 1][0]:
                beta[j_backward,] = 1  # init beta[Xj,] to 1 if last letter of new word
        else:
            #inter_forward = np.dot(X_train[j_forward][1:], Wj.T)
            #inter_backward = np.dot(X_train[j_backward][1:], Wj.T)

            inter_forward = inter_energies[j_forward]
            inter_backward = inter_energies[j_backward]

            for s1 in range(Wj.shape[0]):
                for s2 in range(Wj.shape[0]):
                    alpha[j_forward, s1] += alpha[j_forward - 1, s2] * np.exp(inter_forward[s1] + Tij[s2, s1])
                    beta[j_backward, s2] += beta[j_backward + 1, s1] * np.exp(inter_backward[s2] + Tij[s1, s2])

    stop = timeit.default_timer()
    print('one pass (s): ' + str(stop - start))
    return alpha, beta
...
def conditional_prob(X_train, Wj, Tij):
    ...
    alpha, beta = forward_backward_pass(X_train, Wj, Tij)
